...
const CartPage = () => {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#faf7f2' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.25rem', padding: '4rem 2rem' }}>
          ...
          <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '2rem', color: '#2c1810', margin: 0 }}>Your cart is empty</h2>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#faf7f2' }}>
      ...
      <div style={{ flex: 1, maxWidth: 1100, margin: '0 auto', width: '100%', padding: '2.5rem 1.5rem 5rem' }}>
        {/* Back + Title */}
        <button style={btnStyle.back} onClick={() => navigate('/products')}>
          <ArrowLeft size={15} /> Continue Shopping
        </button>

        <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '2.4rem', fontWeight: 700, color: '#2c1810', margin: '0 0 2rem' }}>
          Shopping Cart <span style={{ fontSize: '1rem', color: '#9a8070', fontFamily: 'sans-serif', fontWeight: 400 }}>({itemCount} item{itemCount !== 1 ? 's' : ''})</span>
        </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '2rem', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {cart.map((item, index) => {
              // ✅ Fix: support both item.image and item.image_url field names
              const imgSrc = fixImageUrl(item.image || item.image_url || '');
              return (
                <motion.div key={`${item.product_id}-${item.color || index}`}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '.5rem' }}>
                      <div>
                        <h3 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.1rem', fontWeight: 700, color: '#2c1810', margin: '0 0 .25rem' }}>
                          {item.product_name}
                        </h3>
                        {item.color && (
                      ...
                      <button onClick={() => removeItem(item.product_id, item.color)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d4c5b0', padding: '.25rem', borderRadius: '6px', display: 'flex', transition: 'color .15s' }}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem' }}>
                        <button style={qtyBtnStyle} onClick={() => updateQuantity(item.product_id, item.color, item.quantity - 1)}><Minus size={14} /></button>
                        <span style={{ fontSize: '1.1rem', fontWeight: 700, color: '#2c1810', minWidth: 28, textAlign: 'center', fontFamily: 'sans-serif' }}>{item.quantity}</span>
                        <button style={qtyBtnStyle} onClick={() => updateQuantity(item.product_id, item.color, item.quantity + 1)}><Plus size={14} /></button>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.3rem', fontWeight: 700, color: '#2c1810' }}>
                          ₹{(item.price * item.quantity).toLocaleString('en-IN')}
                        </div>
                        <div style={{ fontSize: '.75rem', color: '#9a8070', fontFamily: 'sans-serif' }}>₹{item.price.toLocaleString('en-IN')} each</div>
                      </div>
                    </div>
                  </div>
                </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}
            style={{ background: '#fff', borderRadius: '20px', border: '1px solid #e8dfd0', padding: '1.75rem', position: 'sticky', top: '1.5rem' }}>
            <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.4rem', fontWeight: 700, color: '#2c1810', margin: '0 0 1.5rem' }}>Order Summary</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '.6rem', marginBottom: '1.25rem' }}>
              ...
              <div style={{ borderTop: '1px solid #e8dfd0', paddingTop: '1rem', marginTop: '.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.6rem', fontWeight: 700, color: '#2c1810' }}>₹{subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
            </div>

            <button style={{ ...btnStyle.dark, width: '100%', padding: '1rem', fontSize: '1rem', borderRadius: '12px', marginBottom: '.75rem' }}
              onClick={() => navigate('/checkout')}>
              Proceed to Checkout →
            </button>
            <button style={{ width: '100%', padding: '.8rem', background: 'transparent', border: '1.5px solid #e8dfd0', borderRadius: '12px', color: '#9a8070', fontFamily: 'sans-serif', fontSize: '.88rem', fontWeight: 700, cursor: 'pointer', transition: 'all .15s' }}
            ...
            <div style={{ marginTop: '1.25rem', padding: '.85rem', background: '#faf7f2', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '.5rem' }}>
              ...
              <span style={{ fontSize: '.75rem', color: '#9a8070', fontFamily: 'sans-serif', lineHeight: 1.4 }}>Secure checkout. Your information is always protected.</span>
            </div>
          </motion.div>
        </div>
      </div>
...
const cartItemStyle = {
  ...
  border: '1px solid #e8dfd0', display: 'flex', gap: '1.1rem', alignItems: 'flex-start',
};
const imgStyle = {
  width: 100, height: 100, borderRadius: '12px', objectFit: 'cover',
  flexShrink: 0, border: '1px solid #e8dfd0', background: '#f2ede4',
};
const qtyBtnStyle = {
...
const btnStyle = {
  dark: { background: '#2c1810', color: '#fff', border: 'none', padding: '.75rem 1.5rem', borderRadius: '10px', fontFamily: 'sans-serif', fontSize: '.9rem', fontWeight: 700, cursor: 'pointer', transition: 'background .2s', display: 'inline-flex', alignItems: 'center', gap: '.4rem' },
  back: { background: 'none', border: 'none', cursor: 'pointer', color: '#9a8070', fontFamily: 'sans-serif', fontSize: '.88rem', display: 'inline-flex', alignItems: 'center', gap: '.35rem', padding: '0', marginBottom: '1.5rem', transition: 'color .15s' },
};
